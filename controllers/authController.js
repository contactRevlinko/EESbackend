...
const updateAdminPermissions = async (req, res) => {
  try {
    ...
    if (!userId || !Array.isArray(permissions)) {
      return res.status(400).json({ success: false, message: "Invalid input. userId and permissions array are required." });
    }
    const updatedUser = await UserModel.findByIdAndUpdate(userId, { permissions: permissions }, { new: true, runValidators: true });
    if (!updatedUser) return res.status(404).json({ success: false, message: "User not found" });
    return res.status(200).json({ success: true, message: "Admin permissions updated successfully", user: updatedUser });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "An error occurred while updating permissions", error: error.message });
  }
};
